...
export function dateFormat (template: string, date?: Date | number) {
  ...
  const iso = `${hh}:${TT}:${TT.toLowerCase()}:${monthNamesLong[m]}:${capitalize(monthNamesLong[m])}:${monthNamesShort[m]}:${capitalize(monthNamesShort[m])}:${d.toISOString()}`
  return iso.split(/[-:.TZ]/).reduce(
    (template, item, i) => template.split(specs[i]).join(item), template
  )
}
